...
qs_disponibles = df["Q"].unique()

with col1:
    filtro_q = st.multiselect("Filtrar por trimestre (Q)", qs_disponibles, default=qs_disponibles)
with col2:
    filtro_mes = st.multiselect("Filtrar por mes", meses_disponibles, default=meses_disponibles)

df_filtrado = df[df["Q"].isin(filtro_q) & df["Mes"].isin(filtro_mes)]
...
kpi1.metric("Total Meta", f"S/ {df_filtrado['Meta'].sum():,.2f}")
kpi2.metric("Total Facturado", f"S/ {df_filtrado['Facturado'].sum():,.2f}")
kpi3.metric("Total Backlog", f"S/ {df_filtrado['Fac_backlog'].sum():,.2f}")
kpi4.metric("GAP Total", f"S/ {df_filtrado['GAP_calc'].sum():,.2f}")
